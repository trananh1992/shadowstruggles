...
public class ScreenUtilsTest {
	...
	public void defineButtonTest() {
		...
		final float x= MathUtils.random(0,100), y=MathUtils.random(0,100), 
				width= MathUtils.random(0,100), height= MathUtils.random(0,100);
		button = ScreenUtils.defineButton(button, x, y, width, height, skin);
		float [] expected= {x,y,width,height};
		float [] actuals={button.getX(),button.getY(),button.getWidth(),button.getHeight()};
	...
	public void definelabelTest() {
		...
		FileHandle file =Gdx.files.internal("data/images/controls/skin.atlas");
		...
		float [] expected= {x,y,width,height};
		float [] actuals={label.getX(),label.getY(),label.getWidth(),label.getHeight()};
		assertEquals(expected, actuals);	
	}
