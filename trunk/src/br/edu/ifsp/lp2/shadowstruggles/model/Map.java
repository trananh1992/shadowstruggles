		...
		for (Array<Array<Card>> lanes : tiles) {
			for (Array<Card> tiles : lanes) {
				for (Card card : tiles) {
					...
					if (card.direction == 1)
						if (((Fighter) card).getTile() + ((Fighter) card).getRange() > 35)
							lane = card.getLane();
				}
			}
		}
	...
	public boolean playerBaseAttacked() {
		...
		for (Array<Array<Card>> lanes : tiles) {
			for (Array<Card> tiles : lanes) {
				for (Card card : tiles) {
					if (card.direction == -1)
						if (((Fighter) card).getTile() - ((Fighter) card).getRange() < 1)
							return true;
				}
			}
		}
		...
		for (Card fighter : enemies) {
			if (fighter.getClass() == Fighter.class) {
				if (lanes.containsKey(fighter.getLane()))
					lanes.put(fighter.getLane(), lanes.get(fighter.getLane()) + 1);
				else
