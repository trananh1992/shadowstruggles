...
public class InGameMenu extends BaseScreen{
	...
	public InGameMenu(ShadowStruggles game, Controller controller, BattleScreen battleScreen) {
	...
	private void initComponents() {
		...
		returnToGame = new TextButton(game.getManager().getMenuText().returnToGame, getSkin());
		returnToGame=ScreenUtils.initButton(returnToGame, 230, 500, 500, 100);		
		...
		checkCards = new TextButton(game.getManager().getMenuText().checkCards, getSkin());
		checkCards=ScreenUtils.initButton(checkCards, 230, 380, 500, 100);		
		final InGameMenu menu = this;
		checkCards.setClickListener(new ClickListener() {

			@Override
			public void click(Actor arg0, float arg1, float arg2) {
				game.setScreenWithTransition(new CheckCardsScreen(game, controller,menu));

			}
		});
		config = new TextButton(game.getManager().getMenuText().configurations, getSkin());
		config=ScreenUtils.initButton(config, 230, 260, 500, 100);		
		
		config.setClickListener(new ClickListener() {

			@Override
			public void click(Actor arg0, float arg1, float arg2) {
				game.setScreenWithTransition(new ConfigurationScreen(game, controller,menu));

			}
		});
		
		exitGame = new TextButton(game.getManager().getMenuText().exit, getSkin());
		exitGame = ScreenUtils.initButton(exitGame,230,80,500,100);
